-- ============================================================
--  StockVault — Fully Normalized Schema (BCNF, 19 tables)
--  Target: PostgreSQL 15.18 (Docker), run via Beekeeper Studio
--  Phase 3 complete: all 4 BCNF violations resolved.
--  ID strategy: UUID on INVESTOR/BROKER (enumeration resistance);
--               BIGINT IDENTITY elsewhere (write-throughput on
--               high-volume tables).
-- ============================================================
...
-- -------------------------------------------------------
-- 6. BANK_BRANCH   [NEW — resolves BCNF violation in BANK_ACC]
-- -------------------------------------------------------
...
-- -------------------------------------------------------
-- 7. BANK_ACC   [MODIFIED — BANK_NAME/BRANCH_NAME removed]
-- -------------------------------------------------------
CREATE TABLE BANK_ACC (
    BANK_ACC_ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ACC_NO      VARCHAR(20) NOT NULL UNIQUE,
    IFSC_CODE   CHAR(11) NOT NULL REFERENCES BANK_BRANCH(IFSC_CODE) ON DELETE RESTRICT,
...
-- -------------------------------------------------------
-- 9. FUND_TRANSACTION  (associative entity: TRADING_ACC <-> BANK_ACC)
-- -------------------------------------------------------
...
-- -------------------------------------------------------
-- 10. PLAN_CATALOG   [NEW — resolves BCNF violation in BROKER_CLIENT]
-- -------------------------------------------------------
CREATE TABLE PLAN_CATALOG (
    PLAN_TYPE         VARCHAR(50) PRIMARY KEY,
    BROKERAGE_PERCENT NUMERIC(8,4) NOT NULL CHECK (BROKERAGE_PERCENT >= 0)
);

-- -------------------------------------------------------
-- 11. BROKER_CLIENT   [MODIFIED — BROKERAGE_PERCENT removed,
--      PLAN_TYPE is now a FK into PLAN_CATALOG]
-- -------------------------------------------------------
CREATE TABLE BROKER_CLIENT (
    ...
    PLAN_TYPE         VARCHAR(50) REFERENCES PLAN_CATALOG(PLAN_TYPE) ON DELETE SET NULL,
...
-- -------------------------------------------------------
-- 16. HOLDING  (associative entity: PORTFOLIO <-> SECURITY, M:N)
-- -------------------------------------------------------
...
-- -------------------------------------------------------
-- 17. ORDER_RECORD   [MODIFIED — INVESTOR_ID removed]
-- -------------------------------------------------------
...
-- -------------------------------------------------------
-- 19. CAPITAL_GAINS_RECORD   [MODIFIED — GAIN_TYPE removed]
-- -------------------------------------------------------
...
-- ============================================================
-- End of StockVault fully normalized schema (BCNF, 19 tables)
-- ============================================================
